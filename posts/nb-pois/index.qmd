...
The standard form of the negative binomial has two parameters $r$ and $p$ and has the probability mass function (pmf):
$$
  f(y; r, p) = \binom{r + y - 1}{y} \times (1 - p)^{y} \times p^{r}
$$

This distribution has the mean $\mu = \frac{r(1-p)}{p}$ and variance $\sigma^2 = \frac{r(1-p)}{p^{2}}$.
Although, $r$ is sometimes interpreted as the "number of successes" that must be reached, the distribution itself can be generalized to non-integer positive values of $r$ by expressing the binomial coefficient in terms of the $\Gamma$ function.
To see this note that for positive-integer values of $r$:
$$
  \binom{r + y - 1}{y} = \frac{(r + y - 1)!}{(r - 1)! \cdot y!} .
$$
Since the $\Gamma$ function is the generalization of the factorial we can extend the binomial distribution to non-integer positive $r$ by writing:
$$
...
**Proposition:** For a constant $\mu$ as $r \rightarrow \infty$, $f(y; r, \mu)$ converges to a Poisson probability mass function with mean $\mu$, that is:
$$
...
**Proof:**
Starting with the reparameterized pmf, multiply and divide by $\theta^{y}$ and rearrange:
$$
  f(y; r, \mu) = \frac{\Gamma(r + y)}{\Gamma(r)\cdot r^{r}} \times \left(\frac{r}{r + \mu}\right)^{y + r} \frac{\mu^y}{y!} .
$$
